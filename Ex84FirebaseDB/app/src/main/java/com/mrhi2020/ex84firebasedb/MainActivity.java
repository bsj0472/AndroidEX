...
public class MainActivity extends AppCompatActivity {
    ...
    //Friebase 실시간 Database - MySQL같은 RDBMS(테이블형태로 저장하는)와는 다른 방식으로 저장 , NO-SQL 방식
    // node 라는 개념을 이용해서 값들을 저장
    // 이 앱에 Firebase 라이브러리 적용하기
    // Firebase console 사이트의 작업절차[워크플로]를 따라 적용
    ...
    public void clickSend(View view) {
        ...
        //데이터 베이스의 최상위(root) 노드 참조객체 가져오기
        ...
        //DB의 값이 변경된것을 읽어오기- 실시간DB이기에 리스너를 이용함 [별도의 GET메소드 없음]
//        rootRef.addValueEventListener(new ValueEventListener() {
//            @Override
//            public void onDataChange(@NonNull DataSnapshot snapshot) {
//                //변경된 순간의 값을 촬영하다는 느낌의 코드
//                //DataSnapShot 객체가 그 순간의 값을 촬영하고 있다
//                //String data= (String)snapshot.getValue();
//
//                //형변환 코드가 지저분해서 싫다면.. 원하는 자료형.class 를 미리 지정
...
//        });
        ...
        // rootRef 노드의 자식으로 새로운 node를 추가하며 값을 설정
//        DatabaseReference dataRef= rootRef.push(); //새로운 자식노드 추가되고 그 추가된 자식노드의 참조객체 리턴
...
//            @Override
//            public void onDataChange(@NonNull DataSnapshot snapshot) {
//                //snapshot에게 값을 읽어오게 하면 에러
//                //String datga= snapshot.getValue(String.class);
//
...
//        });
        ...
        //3) 최상위 노드 아래 각 노드들에 식별 키값을 부여하면서 데이터 관리하기
...
//        DatabaseReference itemRef= memberRef.push();// "member"노드 아래에 임이의 식별자를 가진 자식노드 생성
...
//        });
        ...
        //5) 나만의 클래스 객체를 만들어서 한번에 멤버값들 저장하기
        // 이름, 나이, 주소를 멤버로 가지는 PersonVO 객체
        String name= et.getText().toString();
        int age= 20;
        String address= "seoul";

        //저장할 값을 가진 PersonVO객체 생성
        PersonVO person= new PersonVO(name, age, address);

        //'persons' 라는 이름의 자식노드 생성 or 참조
        DatabaseReference personRef= rootRef.child("persons");
        personRef.push().setValue(person); //Person객체를 통으로 저장 - 멤버변수명으로 자식노드들 생김
        ...
        personRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                StringBuffer buffer= new StringBuffer();
                for(DataSnapshot snap : snapshot.getChildren() ){
                    PersonVO person= snap.getValue(PersonVO.class);//노드의 값들을 PersonVO객체로 얻어오기
